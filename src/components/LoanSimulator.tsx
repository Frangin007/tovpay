...
export default function LoanSimulator() {
  ...
  function fmtRelative(iso: string) {
    ...
    if (diffMin < 60) return t.relative.minutesAgo(diffMin)
    const diffH = Math.floor(diffMin / 60)
    if (diffH < 24) return t.relative.hoursAgo(diffH)
    return t.relative.daysAgo(Math.floor(diffH / 24))
  }
    ...
    <section className="py-20 px-[5%] bg-white">
      <div className="max-w-[1280px] mx-auto">
        <motion.div className="text-center mb-14" initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <motion.span className="section-tag" variants={fadeUp} custom={0}>{t.eyebrow}</motion.span>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          >
            <div className="mb-8">
              <label className="block text-navy text-xs font-semibold mb-3 uppercase tracking-wide">{t.productLabel}</label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {PRODUCTS.map(p => {
                  const label = labels[p.id as keyof typeof labels]
                  return (
                    <motion.button
                      key={p.id}
                    ...
                    >
                      ...
                      <span className={`block text-xs font-normal mt-0.5 ${p.id === productId ? 'text-white/75' : 'text-g400'}`}>
                        {label.durationLabel} · {(p.rate * 100).toFixed(0)}%
                      </span>
                    </motion.button>
                  )
                })}
            ...
            <div>
              <label className="block text-navy text-xs font-semibold mb-3 uppercase tracking-wide">{t.amountLabel}</label>
              ...
              <div className="flex justify-between text-g400 text-xs">
                ...
                <span>{t.plafondLabel(`${fmt(PLAFOND)} FCFA`)}</span>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="bg-navy rounded-3xl p-8 text-white">
              <div className="text-white/55 text-xs mb-1">{t.result.amountBorrowed}</div>
              <div className="font-display font-bold text-2xl mb-5">{fmt(amount)} FCFA</div>

              <div className="flex justify-between items-center py-3 border-b border-white/10">
                <span className="text-white/55 text-sm">{t.result.interest}</span>
              ...
              <div className="flex justify-between items-center py-3 border-b border-white/10">
                <span className="text-white/55 text-sm">{t.result.duration}</span>
                <span className="font-semibold">{labels[product.id as keyof typeof labels].durationLabel}</span>
              </div>
              <div className="flex justify-between items-center py-3 border-b border-white/10">
                <span className="text-white/55 text-sm flex items-center gap-1.5"><CalendarIcon size={14} /> {t.result.dueDate}</span>
                <span className="font-semibold">{fmtDate(result.dueDate)}</span>
              </div>

              <div className="flex justify-between items-center pt-5 mt-2">
                <span className="text-white/70 font-semibold">{t.result.total}</span>
                <motion.span
                  key={result.total}
              ...
              <div className="mt-5 pt-4 border-t border-white/10 text-white/40 text-[11px] leading-relaxed">
                {t.result.annualRate(result.effectiveAnnualRate.toFixed(0))}
              </div>
              <div className="mt-2 text-white/40 text-[11px] leading-relaxed">
                {t.result.latePenalty}
              </div>
            </div>
              ...
              <div className="bg-g50 rounded-2xl border border-g100 overflow-hidden">
                ...
                >
                  <span className="text-sm font-semibold text-navy">
                    {t.schedule.toggle(product.installments ?? 0, fmt(result.dailyPayment ?? 0))}
                  </span>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="text-g400 text-xs uppercase tracking-wide">
                              <th className="text-left font-semibold py-1.5">{t.schedule.colIndex}</th>
                              <th className="text-left font-semibold py-1.5">{t.schedule.colDate}</th>
                              <th className="text-right font-semibold py-1.5">{t.schedule.colAmount}</th>
                            </tr>
                          </thead>
                          <tbody>
            ...
            <Link
              to="/contact"
              state={{ simulation: { productName: labels[product.id as keyof typeof labels].name, amount, total: result.total, dueDateLabel: fmtDate(result.dueDate) } }}
            >
              <motion.span
        ...
        <motion.div className="mt-16" initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <motion.h3 className="font-display font-bold text-navy text-xl mb-6 text-center" variants={fadeUp} custom={0}>
            {t.compareTitle(fmt(Math.min(amount, PLAFOND)))}
          </motion.h3>
            ...
            {PRODUCTS.map((p, i) => {
              ...
              return (
                <motion.button
                  ...
                  custom={i + 1}
                >
                  <div className="text-navy font-semibold text-sm mb-3">{label.name}</div>
                  ...
                  <div className="text-xs text-g400 mb-3">{t.interestOf(fmt(r.interest))}</div>
                  <div className="text-xs font-semibold" style={{ color: p.color }}>{label.durationLabel}</div>
                </motion.button>
              )
            })}
          </div>
        ...
        {history.length > 0 && (
          <motion.div className="mt-14" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            <h4 className="text-navy text-xs font-semibold uppercase tracking-wide mb-4 text-center">{t.historyTitle}</h4>
            <div className="flex flex-wrap justify-center gap-3">
              {history.map(h => {
                const histLabel = labels[h.productId as keyof typeof labels]
                return (
                  <div key={h.id} className="flex items-center gap-3 bg-g50 border border-g100 rounded-xl px-4 py-2.5 text-sm">
                    <span className="font-semibold text-navy">{histLabel ? histLabel.name : h.productName}</span>
